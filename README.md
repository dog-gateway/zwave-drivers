...
The main configuration files required to succesfully operate z-wave drivers are the following:
...
A typical single driver configuration file contains one line, only, setting the polling time for the "represented" family of devices, i.e., all the dimmer lamps. A sample is reported below.
...
Dog zwave-drivers, and in particular, the Dog zwave-gateway exploits a simple, file-based, database of devices which maps real devices to corresponding Dog (and thus [DogOnt](http://iot-ontologies.github.io/dogont)) device classes. Real devices are identified by means of a unique triple composed of ```ManufacturerId(decimal)- Device type(decimal) - Device id (decimal)```. The bundled device db can be easily estended by adding new lines using the same format ```<triple = Dog class>``` provided that an existing class is defined which can be mapped to the new device. For example, the full [set](https://github.com/OpenZWave/open-zwave/blob/master/config/manufacturer_specific.xml) of devices supported in [OpenZwave](https://github.com/OpenZWave) can be easily added, paying attention to the fact that in OpenZwave the above IDs are represented as hexadecimal numbers.
...
Dog supports discovery of zwave-devices in the association / disassociation phase, only. However, to successfully detect association/disassociation a connection towards one ore more ZWay servers shall be configured. This requires to manually add a "gateway" section in the Dog environment configuration files. A sample gateway configuration section is reported below.
...
- notifications:
    ...
    - generatesCommand: []
        ...
        value: "associating"
        id: "AssociatingDeviceNotification_zwave-gateway"
        class: "AssociatingDeviceNotification"
    - generatesCommand: []
        param:
        - name: "nParams"
        value: "0"
        ...
        id: "DisassociatingDeviceNotification_zwave-gateway"
        class: "DisassociatingDeviceNotification"
    class: "AssociationNotificationFunctionality"
state: []
id: "zwave-gateway"
domoticSystem: "ZWave"
class: "ZWaveGateway"
...
**WARNING** Legacy XML configuration is deprecated and reported below only for the sake of clarity.
